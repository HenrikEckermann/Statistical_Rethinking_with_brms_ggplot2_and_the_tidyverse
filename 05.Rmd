...
fitted(b5.3, newdata = nd) %>% 
  ...
  # `rename()` them to keep them straight.
  rename(f_ll = Q2.5,
         f_ul = Q97.5) %>% 
...
```{r b5.Os, cache = T, message = F, warning = F, results = 'hide'}
b5.O.both <- 
  brm(data = d, family = gaussian,
      y ~ 1 + x_pos + x_neg,
      prior = c(prior(normal(0, 100), class = Intercept),
...
That posterior $SD$ looks much better. Compare this density to the one in Figure 5.8.b.
...
```{r b5.10_and_b5.11, cache = T, message = F, warning = F, results = 'hide'}
...
McElreath wrote "these two variables form essentially a single axis of variation" (p. 148). You can really see that on the lower two scatter plots. You'll note the `ggpairs()` plot also showed the Pearson's correlation coefficients.
...
```{r b5.13, cache = T, message = F, warning = F}
b5.13 <- 
  brm(data = d, family = gaussian,
...
```{r b5.14, cache = T, message = F, warning = F, results = 'hide'}
b5.14 <- 
  update(b5.13, 
...
```{r b5.15, cache = T, message = F, warning = F}
b5.15 <- 
  brm(data = d, family = gaussian,
      height ~ 1 + male,
...
```{r b5.15b, cache = T, message = F, warning = F}
b5.15b <- 
  brm(data = d, family = gaussian,
...
```{r b5.16_alt, cache = T, message = F, warning = F}
b5.16_alt <- 
  brm(data = d, family = gaussian,
      kcal.per.g ~ 0 + clade,
